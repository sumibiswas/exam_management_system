...
<i><p style="color:white; line-height: 0%; font-size: 14px;"><?php echo"$slogan"; ?></p></i>
...
<div>
    <h1 style="font-size: 50px; text-align: right;">
    <p id="counttdown"></p>
    </h1>
...
<form action="submit_exam.php" method="GET" id="form_submit">
...
if ($result->num_rows > 0) {
    ...
    echo "<tr><td>$quesno)  " . $row["question"]. "</td>";
    $i++;
	...
	print '<tr style="display:none;"><td><input type="radio"  Checked name="q'.$quesno.'" value="'.$row["option5"].'" required="true"> '.$row["option5"].'<br></td></tr>';
...
<button type="submit" id="btn_save" onclick="return confirm('Are you sure you want to submit your assessment ?')"  class="btn btn-primary">Submit Assessment</button><br><br>
</form>
...
<script type="text/javascript">
    ...
    const counttdownEl=document.getElementById('counttdown');   

    setInterval(updateCuntdown, 1000);
    function updateCuntdown(){
let minutes=Math.floor(time/60);
let second=time%60;
counttdownEl.innerHTML=`0${minutes}: ${second}`;
        time--;
